...
class Lifting_down(nn.Module):
    ...
    def regular_term_loss(self):
        ...
        constraint1 = torch.sum(torch.pow(torch.sum(self.low_pass_filter_h, dim = 3) - 1, 2) +\
                                torch.pow(torch.sum(self.low_pass_filter_v, dim = 2) - 1, 2), dim = 0).squeeze(-1)
...
# =============================================================================
        ...
        constraint2 = torch.sum(torch.sum(self.high_pass_filter_h, dim = 3) + torch.sum(self.high_pass_filter_v, dim = 2) +\
                      torch.pow(1 - torch.sum(torch.pow(self.high_pass_filter_h, 2), dim = 3), 2) +\
                      torch.pow(1 - torch.sum(torch.pow(self.high_pass_filter_v, 2), dim = 2), 2), dim = 0).squeeze(-1)
            
# =============================================================================
#         # constraint3 => H + L = 1
#         vertical_sum = torch.sum(self.low_pass_filter_v, dim = 2) + torch.sum(self.high_pass_filter_v, dim = 2)
#         horizontal_sum = torch.sum(self.low_pass_filter_h, dim = 3) + torch.sum(self.high_pass_filter_h, dim = 3)
#         constraint3 = torch.sum(torch.pow(1 - vertical_sum, 2) + torch.pow(1 - horizontal_sum, 2), dim = 0).squeeze(-1)
# =============================================================================
        return (constraint1 + constraint2).squeeze(-1)
