...
class Lifting_down(nn.Module):
    def __init__(self, channel, kernel_size = 2, stride = None, pad_mode = 'discard', pad_place = [0, 1, 0, 1], energy_filter = None):
        ...
        if self.energy_filter is not None:
            self.energy_filter1 = self.energy_filter(channel)
            self.energy_filter2 = self.energy_filter(channel)
            self.energy_filter3 = self.energy_filter(channel)
            self.energy_filter4 = self.energy_filter(channel)
    ...
    def __repr__(self):
        if self.energy_filter is None:
            struct = "Lifting({}, kernel_size={}, stride={}, pad_mode={})".format(self.channel, self.kernel_size, self.stride, self.pad_mode)
        else:
            struct = "Lifting({}, kernel_size={}, stride={}, pad_mode={}, energy_filter=True)".format(self.channel, self.kernel_size, self.stride, self.pad_mode)
...
# =============================================================================
    ...
    def forward(self, x):
        ...
        if self.energy_filter is not None:
            x_ll = self.energy_filter1(x_ll)
            x_hl = self.energy_filter2(x_hl)
            x_lh = self.energy_filter3(x_lh)
            x_hh = self.energy_filter4(x_hh)
