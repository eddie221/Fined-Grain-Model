...
def train_step(model, data, label, loss_func, optimizers, phase):
    ...
    #loss function
    cls_loss = loss_func[0](output_1[0], b_label)# + loss_func[0](output_2, b_label)
    for i in range(1, len(output_1)):
        cls_loss = cls_loss + loss_func[0](output_1[i], b_label) / 3
        
    cls_loss = cls_loss / 2 + loss_func[0](output_cha_2.view(-1, NUM_CLASS), b_label.repeat_interleave(output_cha_2.shape[1])) / 2

    er_loss = torch.mean(torch.abs(cam_1.view(cam_1.shape[0], -1) - cam_rf_1.view(cam_rf_1.shape[0], -1)))

    loss = cls_loss + er_loss
    
    total_probability = torch.cat([output_2.unsqueeze(1), output_cha_2], dim = 1)
    total_probability = torch.sum(total_probability.data, dim = 1)
    _, predicted = torch.max(total_probability, dim = 1)
